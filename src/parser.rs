use crate::{JsonValue, JsonValueLazy, Number, ParseError};
use std::collections::HashMap;

pub(crate) fn parse_value(s: &str) -> Result<JsonValue, ParseError> {
    Ok(
        if s.starts_with('"') {
...
pub(crate) fn parse_obj(tokens: &[String], pos: usize) -> Result<(usize, JsonValue), ParseError> {
...
pub(crate) fn parse_arr(tokens: &[String], pos: usize) -> Result<(usize, JsonValue), ParseError> {
    let (first_pos, mut pos) = (pos, pos+1);
...
pub(crate) fn parse_obj_lazy(tokens: &[String], pos: usize) -> Result<(usize, JsonValueLazy), ParseError> {
    let mut result = HashMap::new();
    let mut key = None;
    let (first_pos, mut pos) = (pos, pos+1);
    ...
    loop {
        ...
        match token.as_str() {
            "{" => {
                let (diff, val) = parse_obj_lazy(tokens, pos)?;
                result.insert(key.take().unwrap(), val);
                pos += diff;
            }
            "}" => {
                return Ok((pos - first_pos, JsonValueLazy::Object(result)));
            }
            "[" => {
                let (diff, val) = parse_arr_lazy(tokens, pos)?;
                result.insert(key.take().unwrap(), val);
                pos += diff;
            }
            ":" | "," => {}
            _ => {
                if key.is_none() {
                    ...
                    result.insert(key.take().unwrap(), JsonValueLazy::Something(token.clone()));
                }
            }
        };
        pos += 1;
...
pub(crate) fn parse_arr_lazy(tokens: &[String], pos: usize) -> Result<(usize, JsonValueLazy), ParseError> {
    let (first_pos, mut pos) = (pos, pos+1);
    let mut result = Vec::with_capacity(tokens.len()/2);
    ...
    loop {
        ...
        match token.as_str() {
            ...
            "]" => {
                return Ok((pos - first_pos, JsonValueLazy::Array(result)));
            }
            _ => {
                result.push(JsonValueLazy::Something(token.clone()));
            }
        }
        pos += 1;
    }
...
pub fn parse_lazy(s: &str) -> Result<JsonValueLazy, ParseError> {
    Ok(parse_obj_lazy(&s.split('"')
        .fold((Vec::new(), String::new(), true), |state, s| {
        let (mut state, mut current, odd) = state;
        if s.ends_with('\\') {
            current.reserve_exact(s.len() + 1);
